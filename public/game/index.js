...
function showPathwayFeedback() {
    let pathwayMessage = "";
    if (totalPoints >= 12) {
        pathwayMessage = "You're on an excellent financial path! Keep up the great decision-making.";
    } else if (totalPoints >= 8) {
        pathwayMessage = "You're making good financial decisions. There's room for improvement, but you're on the right track.";
    } else if (totalPoints >= 4) {
        pathwayMessage = "You're making some good choices, but there's significant room for improvement in your financial decision-making.";
    } else {
        pathwayMessage = "You might want to reconsider your financial strategies. Try to prioritize long-term goals and make more informed decisions.";
    }

    showFeedback(pathwayMessage);
    setTimeout(hideFeedback, 3000);  // Show pathway feedback for 3 seconds
}
...
function checkAnswer(key) {
    ...
    const selectedOption = currentEvent.options[key.charCodeAt(0) - 65];
    
    totalPoints += selectedOption.points;

    showFeedback(`You chose: ${selectedOption.text}\n\nBest option: ${currentEvent.explanation}\n\nYour current total points: ${totalPoints}`);
...
const events = [
    {
        scenario: "Unexpected Expense: Your smartphone screen cracks. Repairing it will cost $200.",
        options: [
            { text: "Use your emergency fund to fix the phone immediately.", points: 2 },
            { text: "Use your laptop savings to fix the phone and adjust your goal timeline.", points: 1 },
            { text: "Use a backup old phone and continue saving for the laptop.", points: 3 },
            { text: "Take out a small loan from your parents to fix the phone.", points: 0 }
        ],
        explanation: "Using a backup phone is the best option as it prioritizes your long-term goal and finds a creative solution to the immediate problem."
    },
    {
        scenario: "Value-Based Decision: Your school announces a student exchange program to Japan, costing $2,000.",
        options: [
            { text: "Abandon your laptop goal and save for the trip instead.", points: 0 },
            { text: "Stick to your laptop goal and dismiss the trip idea entirely.", points: 1 },
            { text: "Extend your saving timeline to accommodate both goals.", points: 2 },
            { text: "Look for ways to reduce the trip's cost and raise additional funds.", points: 3 }
        ],
        explanation: "Looking for ways to reduce costs and raise funds is the best option as it applies value-based decision making and seeks a balance between different goals."
    },
    {
        scenario: "Opportunity Cost: The laptop you want is on sale for $800, a $200 discount. You've only saved $600 so far.",
        options: [
            { text: "Use your savings and borrow $200 to buy the laptop now.", points: 0 },
            { text: "Use your savings and cut your monthly budget to make up the $200.", points: 3 },
            { text: "Wait and continue with your original savings plan.", points: 1 },
            { text: "Look for a part-time job to earn the extra $200 quickly.", points: 2 }
        ],
        explanation: "Using your savings and cutting your monthly budget is the best option as it captures the discount opportunity while promoting financial discipline."
    },
    {
        scenario: "Compound Growth vs. Immediate Gratification: Your friend suggests investing your $500 savings in a high-yield savings account.",
        options: [
            { text: "Keep the money in your current low-interest savings account.", points: 1 },
            { text: "Invest all $500 in the high-yield savings account.", points: 3 },
            { text: "Invest $250 in the high-yield account and keep $250 in your current account.", points: 2 },
            { text: "Spend $100 on something fun and invest the rest in the high-yield account.", points: 0 }
        ],
        explanation: "Investing all $500 in the high-yield savings account is the best option as it maximizes the potential for compound growth while keeping the money accessible."
    },
    {
        scenario: "Balancing Social Life and Financial Goals: Your friends are planning weekly outings that will cost about $40 each time.",
        options: [
            { text: "Participate every week, reducing your monthly savings.", points: 1 },
            { text: "Skip all outings to maximize your savings.", points: 0 },
            { text: "Suggest cheaper alternatives for hangouts.", points: 3 },
            { text: "Participate every other week and save the difference.", points: 2 }
        ],
        explanation: "Suggesting cheaper alternatives for hangouts is the best option as it maintains social connections while aligning with financial goals."
    }
];
  ...
  function showNotification() {
    ...
    const message = `${currentEvent.scenario}\n\n` + 
...
function hideNotification() {
    ...
    unfreezeChicken();
    currentEventIndex = -1; // Reset for next event
    questionAsked = false;
}
...
const update = () => {
    ...
    if (lanes) {
        ...
        lanes.filter(lane => lane.index >= chicken.getLane() - 9 && lane.index <= chicken.getLane() + 9).forEach(lane => {
            ...
            if ((lane.type == 'field' || lane.type == 'forest') && chicken.getLane() !== chicken.questionAskedLane && !questionAsked) {
                showNotification();
        ...
        if (lanes[chicken.getLane()] && (lanes[chicken.getLane()].type !== 'field' && lanes[chicken.getLane()].type !== 'forest')) {
            hideNotification();
            questionAsked = false; // Reset the question asked flag when moving out of grass lanes
        }
    }
